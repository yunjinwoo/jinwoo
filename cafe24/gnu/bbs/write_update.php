...
if ($w == "" || $w == "r") 
{
	...
	if( empty($board[bo_write_change]) )
	{
		$wr_name = $tmp_wr_name ;
	}
...
else if ($w == "u") 
{
    ...
    {		
        ...
        {
            $mb_id = $wr[mb_id];
            $wr_name = empty($wr_name)?$wr[wr_name] : $wr_name;
    ...
    $sql = " update $write_table
                set ca_name = '$ca_name',
                    ...
                    $sql_ip
                    $sql_password " ;
## 2007-09-06 날짜 조회수 수정기능 코드..
if( empty($board[bo_hit_date_change]) && !empty($is_admin))
{
	$sql .= ", wr_hit='$wr_hit',  wr_datetime='$wr_datetime'" ;	
}
