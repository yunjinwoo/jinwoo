    //입점 브랜드 상세 레이어 팝업
	function floorBrandInfoDetailAjax(brch_flr_brnd_no, korNm, flrCd, flrNm, callbackType){
		
		var frm = document.detailForm;
        frm.str_brch_flr_brnd_no.value   = brch_flr_brnd_no;
        frm.korNm.value   = korNm;
        frm.flrCd.value   = flrCd;
        frm.flrNm.value   = flrNm;
        frm.str_contentGubun.value   = "1";
        frm.action = "/handler/StairGuide_F-BaseInfoSearch";
       
	    var callback = { callback:function(){$(document).triggerHandler("popup_1Action");} } ;
		if( callbackType == '2' )
			callback = { callback:function(){$(document).triggerHandler("popup_2Action");} } ;
		
		$.post( $(frm).attr('action') , $(frm).serialize() , function(data){
			$.__maskAjax_make(data,callback) ;
		});
	}
...
$(function(){
	...
	$(document).bind("popup_1Action", function(){
		// 층별안내 확대 버튼
		//$("div#modal_wrap div.plan a").bind('click', function(e){("popup_2Action");}});
		// 층별안내 전점 입점 정보 보기 링크
		//$("div#modal_wrap div.info table a").bind('click', function(e){("popup_1Action");}});
		
		// 팝업내 페이지
		//$("div#modal_wrap div.info div.paginate2 a").bind('click', function(e){("popup_1Action");}});
	...
	$(document).bind("popup_2Action", function(){
		...
		//$("div#modal_wrap div.pic a.btn_pre").bind('click', function(e){("popup_1Action");}});
	...
	$(document).bind("popup_5Action", function(){
		//$("div#modal_wrap dl.srch_list a").bind('click', function(e){("popup_1Action");}});
	});
